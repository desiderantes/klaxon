...
object Annotations {
    ...
    fun findNonIgnoredProperties(kc: KClass<*>?, strategies: List<PropertyStrategy>): List<KProperty1<out Any, Any?>> =
        findProperties(kc).filter {
            ...
            it.visibility == KVisibility.PUBLIC && !(ignored ?: false) ||
                    it.visibility == KVisibility.PRIVATE && !(ignored ?: true)
        }.filter { property ->
            // PropertyStrategy
...
//            println("JSON PATHS FOR $kc: $result")
    ...
    fun isArray(type: KType?): Boolean {
        return type?.jvmErasure is KClass<*> && type.jvmErasure.java.isArray
    }
